...
function verifyInstall(variant) {
  ...
  if (variant === "online") {
    run(process.execPath, args, { env: { XLITE_NETWORK_ISOLATION: "none" } })
    return
  }
  ...
  if (!unshareArgs) {
    ...
    run(process.execPath, args, { env: { XLITE_NETWORK_ISOLATION: "not-enforced" } })
    return
  }
  run("unshare", [
    ...unshareArgs,
    "sh", "-c",
    "ip link set lo up && export XLITE_NETWORK_ISOLATION=\"$1\" && shift && exec \"$@\"",
...
function main() {
  ...
  if (options.phase === "prepare") prepare(options)
  else if (options.phase === "prepare-assets") console.log("[xlite-ci] no external assets to prepare")
    ...
    env: { XLITE_TEST_VARIANTS: options.variants.join(",") },
  })
...
try {
  ...
  console.error(`[xlite-ci] ${error.message}`)
  process.exit(1)
}