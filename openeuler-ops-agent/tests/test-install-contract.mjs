...
const sandbox = mkdtempSync(join(tmpdir(), "openeuler-ops-install-contract-"))
const TEST_VARIANTS = (process.env.OPENEULER_OPS_TEST_VARIANTS || "online,offline")
...
if (TEST_VARIANTS.length === 0 || TEST_VARIANTS.some((variant) => !["online", "offline"].includes(variant))) {
  throw new Error("OPENEULER_OPS_TEST_VARIANTS must contain online and/or offline")
}
...
function listBackups(configPath) {
  const prefix = `${basename(configPath)}.openeuler-ops-backup-`
...
function exerciseConfigure(project, configPath, environment) {
  ...
  assert(config.agent?.["openeuler-ops"], "configure: openeuler-ops was not registered")
...
function exerciseRemove(project, configPath, environment) {
  ...
  assert(!config.agent?.["openeuler-ops"], "configure remove: openeuler-ops registration remains")
...
`)
  ...
  const environment = {
    ...
    OPENEULER_OPS_OPENCODE_CONFIG: configPath,
