...
function listBackups(configPath) {
  ...
  return existsSync(directory)
    ? execFileSync("find", [directory, "-maxdepth", "1", "-name", `${basename(configPath)}.openeuler-ops-backup-*`], { encoding: "utf8" })
        .trim().split("\n").filter(Boolean).sort()
    : []
}
...
function main() {
  ...
  const workdir = mkdtempSync(join(tmpdir(), `openeuler-ops-real-${options.variant}-`))
...
`
  ...
  writeFileSync(join(projectDir, "package.json"), `${JSON.stringify({ name: `openeuler-ops-${options.variant}-consumer`, private: true }, null, 2)}\n`)
  ...
  try {
    ...
    const report = {
      ...
      packageName,
      networkIsolation: process.env.OPENEULER_OPS_NETWORK_ISOLATION || "none",
