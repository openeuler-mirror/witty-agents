...
function verifyInstall(variant) {
  ...
  if (variant === "online") {
    run(process.execPath, args, { env: { OPENEULER_OPS_NETWORK_ISOLATION: "none" } })
    return
  }
  ...
  if (!unshareArgs) {
    ...
    run(process.execPath, args, { env: { OPENEULER_OPS_NETWORK_ISOLATION: "not-enforced" } })
    return
  }
  run("unshare", [
    ...unshareArgs,
    "sh", "-c",
    "ip link set lo up && export OPENEULER_OPS_NETWORK_ISOLATION=\"$1\" && shift && exec \"$@\"",
...
function main() {
  ...
  if (options.phase === "prepare") prepare(options)
  else if (options.phase === "prepare-assets") console.log("[openeuler-ops-ci] no external assets to prepare")
    ...
    env: { OPENEULER_OPS_TEST_VARIANTS: options.variants.join(",") },
  })
...
try {
  ...
  console.error(`[openeuler-ops-ci] ${error.message}`)
  process.exit(1)
}