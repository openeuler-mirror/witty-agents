...
const OPENEULER_OPS_PACKAGE_SEGMENTS = [
  "agent-openeuler-ops",
  "witty-agent-openeuler-ops",
  "openeuler-agent-ops",
  "-openeuler-ops-online",
  "-openeuler-ops-offline",
]
const SKILLS = [
  "buddy-log-analyzer",
  "cool-agent-tools",
...
function isPluginSpec(spec) {
  ...
  try {
    ...
    return OPENEULER_OPS_PACKAGE_SEGMENTS.some((segment) => pluginPath.includes(segment))
  } catch {
    return false
  }
}
...
export function resolveSkillsDir() {
  if (process.env.OPENEULER_OPS_OPENCODE_CONFIG) {
    return resolve(join(dirname(resolve(process.env.OPENEULER_OPS_OPENCODE_CONFIG)), "skills"))
  }
  const configHome = process.env.XDG_CONFIG_HOME
    ? resolve(process.env.XDG_CONFIG_HOME)
    : join(homedir(), ".config")
