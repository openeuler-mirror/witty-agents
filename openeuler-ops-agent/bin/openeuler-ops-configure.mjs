...
function printUsage() {
  console.log(`Usage:
  openeuler-ops-configure [install] [--target opencode]   # 注册 Agent 到 opencode 配置（幂等，默认动作）
  openeuler-ops-configure remove    [--target opencode]   # 从 opencode 配置移除 Agent（幂等）
  openeuler-ops-configure status    [--target opencode]   # 查看注册状态（JSON）
...
try {
  ...
  console.error(`[openeuler-ops-configure] ${error.message}`)
  process.exit(1)
}
