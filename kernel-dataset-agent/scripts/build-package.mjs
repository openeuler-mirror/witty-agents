#!/usr/bin/env node
// 打包：把包内容复制到 .package-stage/<style>/<variant>，校验必需文件，然后 npm pack。
...
// src/ 是采集器实现，config/ 是默认数据集配置，两者都必须进包。
const COPY_ITEMS = ["agent", "config", "dist", "skills", "bin", "lib", "src", "scripts", "README.md"]
