#!/usr/bin/env node
// kernel-dataset Agent 的 CI 驱动：由 Jenkinsfile 按阶段调用（prepare / prepare-assets /
// install-dependencies / validate / build / artifact-gates / install-contract / real-install）。
...
function prepare(options) {
  ...
  if (options.variants.includes("offline") && !canRun("sh", ["-c", "command -v unshare >/dev/null && command -v ip >/dev/null"])) {
    console.warn("WARNING: offline network-isolation tools are unavailable; offline install is still verified hermetic via --offline")
  }
...
function verifyInstall(variant) {
  ...
  if (variant === "online") {
    run(process.execPath, args, { env: { KERNEL_DATASET_NETWORK_ISOLATION: "none" } })
    return
  }
  ...
  if (!unshareArgs) {
    ...
    run(process.execPath, args, { env: { KERNEL_DATASET_NETWORK_ISOLATION: "not-enforced" } })
    return
  }
  run("unshare", [
    ...unshareArgs,
    "sh", "-c",
    "ip link set lo up && export KERNEL_DATASET_NETWORK_ISOLATION=\"$1\" && shift && exec \"$@\"",
...
function main() {
  ...
  if (options.phase === "prepare") prepare(options)
  else if (options.phase === "prepare-assets") console.log("[kernel-dataset-ci] no external assets to prepare")
  else if (options.phase === "install-dependencies") run("npm", ["ci", "--ignore-scripts", "--no-audit", "--no-fund"])
  else if (options.phase === "validate") run("npm", ["run", "validate"])
    ...
    env: { KERNEL_DATASET_TEST_VARIANTS: options.variants.join(",") },
  })
...
try {
  ...
  console.error(`[kernel-dataset-ci] ${error.message}`)
  process.exit(1)
}