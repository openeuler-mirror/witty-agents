...
const sandbox = mkdtempSync(join(tmpdir(), "vllm-install-contract-"))
const TEST_VARIANTS = (process.env.VLLM_BENCHMARK_TEST_VARIANTS || "online")
  .split(",").map((variant) => variant.trim()).filter(Boolean)

if (TEST_VARIANTS.length === 0 || TEST_VARIANTS.some((variant) => !["online"].includes(variant))) {
  throw new Error("VLLM_BENCHMARK_TEST_VARIANTS must contain online")
}
...
function listBackups(configPath) {
  const prefix = `${basename(configPath)}.vllm-benchmark-backup-`
...
function exerciseConfigure(project, configPath, environment) {
  ...
  execFileSync("npm", ["exec", "--offline", "--", "vllm-benchmark-configure", "install"], {
                    cwd: project, stdio: "inherit", env: environment,
                  })
  ...
  assert(
    Array.isArray(config.plugin) && config.plugin.some((spec) => (
      typeof spec === "string" && spec.endsWith("/dist/index.js") && spec.includes("vllm-benchmark")
    )),
    "configure: vllm-benchmark plugin was not registered",
  )
  ...
  const skillLink = join(skillsDir, "vllm-benchmark")
  assert(existsSync(skillLink) && lstatSync(skillLink).isSymbolicLink(), "configure: vllm skill link is missing")
  assert(existsSync(join(readlinkSync(skillLink), "SKILL.md")), "configure: vllm skill link target has no SKILL.md")
...
function exerciseRemove(project, configPath, environment) {
  const backupsBefore = listBackups(configPath).length
  execFileSync("npm", ["exec", "--offline", "--", "vllm-benchmark-configure", "remove"], {
    cwd: project, stdio: "inherit", env: environment,
  })
  const after = readFileSync(configPath, "utf8")
  const config = parse(after, [], { allowTrailingComma: true, disallowComments: false })
  ...
  assert(
    ...
    "configure remove: vllm-benchmark plugin registration remains",
  )
  ...
  assert(!existsSync(skillLink), "configure remove: vllm skill link was not removed")
...
function exerciseSetup(project, environment) {
  // Setup prepares Python separately from OpenCode registration.
  for (const command of ["install", "check"]) {
    const output = execFileSync(
      "npm", ["exec", "--offline", "--", "vllm-benchmark-setup", command],
      { cwd: project, env: environment, encoding: "utf8" },
    )
    assert(output.includes('"status": "ready"'), `setup ${command}: readiness JSON not reported`)
    assert(output.includes('"backend": "python"'), `setup ${command}: backend must report "python"`)
  }
}
...
`)
  ...
  const environment = {
    ...
    VLLM_BENCHMARK_VENV: join(sandbox, "venv"),
    VLLM_BENCHMARK_DATA_DIR: join(sandbox, "data"),
