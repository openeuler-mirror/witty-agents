...
function printUsage() {
  console.log(`Usage:
  vllm-benchmark-configure [install] [--target opencode]   # 注册 Agent 到 opencode 配置（幂等，默认动作）
  vllm-benchmark-configure remove    [--target opencode]   # 从 opencode 配置移除 Agent（幂等）
  vllm-benchmark-configure status    [--target opencode]   # 查看注册状态（JSON）
...
function parseArgs(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (["-h", "--help", "help"].includes(arg)) {
      ...
      console.error(`[vllm-benchmark-configure] unknown argument: ${arg}\n`);
      printUsage();
      process.exit(1);
    }
  }
  if (!["opencode", "all"].includes(options.target)) {
    console.error("[vllm-benchmark-configure] --target must be one of: opencode, all");
    process.exit(1);
  }
  return options;
}
