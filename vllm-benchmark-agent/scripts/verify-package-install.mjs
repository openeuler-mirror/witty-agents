...
function parseArgs(argv) {
  ...
  if (!options.artifact || !["online"].includes(options.variant)) {
    throw new Error("--artifact and --variant=online are required")
  }
...
function listBackups(configPath) {
  ...
  return existsSync(directory)
    ? execFileSync("find", [directory, "-maxdepth", "1", "-name", `${basename(configPath)}.vllm-benchmark-backup-*`], { encoding: "utf8" })
        .trim().split("\n").filter(Boolean).sort()
    : []
}
...
function main() {
  ...
  const workdir = mkdtempSync(join(tmpdir(), `vllm-real-${options.variant}-`))
...
`
  ...
  writeFileSync(join(projectDir, "package.json"), `${JSON.stringify({ name: `vllm-${options.variant}-consumer`, private: true }, null, 2)}\n`)
  ...
  const environment = {
    ...
    VLLM_BENCHMARK_OPENCODE_CONFIG: configPath,
    VLLM_BENCHMARK_VENV: join(workdir, "venv"),
    VLLM_BENCHMARK_DATA_DIR: join(workdir, "data"),
  ...
  try {
    ...
    assert(existsSync(join(projectDir, "node_modules", ".bin", "vllm-benchmark-setup")), "vllm-benchmark-setup bin is not registered")
    assert(existsSync(join(projectDir, "node_modules", ".bin", "vllm-benchmark-configure")), "vllm-benchmark-configure bin is not registered")
    const setupOutput = run("npm", ["exec", "--offline", "--", "vllm-benchmark-setup", "install"], {
    ...
    run("npm", ["exec", "--offline", "--", "vllm-benchmark-setup", "install"], {cwd: projectDir, env: environment});
    run("npm", ["exec", "--offline", "--", "vllm-benchmark-setup", "check"], {cwd: projectDir, env: environment});
    assert(readFileSync(configPath, "utf8") === initialConfig, "setup changed OpenCode config");
    copyFileSync(join(packageRoot, "benchmark.yaml"), join(projectDir, "benchmark.yaml"));
    const resolvedConfig = JSON.parse(run("npm", ["exec", "--offline", "--", "vllm-benchmark", "config"], {
      cwd: projectDir, env: {...environment, VLLM_BENCHMARK_CONFIG: join(projectDir, "benchmark.yaml")}, encoding: "utf8",
    }));
    assert(resolvedConfig.ok, "installed Python runner cannot load consumer configuration");
    assert(!existsSync(join(packageRoot, ".vllm-benchmark")), "runner wrote state into package");

    const configure = ["exec", "--offline", "--", "vllm-benchmark-configure", "install"]
    ...
    assert(parsed.plugin.includes(expectedPluginSpec), "configure did not register the vllm-benchmark plugin")
    assert(!parsed.agent?.["vllm-benchmark"], "configure must not write a direct config.agent entry (plugin mechanism)")
    ...
    const skillLink = join(dirname(configPath), "skills", "vllm-benchmark")
    assert(lstatSync(skillLink).isSymbolicLink(), "configure did not link the vllm-benchmark skill")
    const linkedSkillRoot = realpathSync(skillLink)
    assert(
      linkedSkillRoot === realpathSync(join(packageRoot, "skills", "vllm-benchmark")),
      "vllm-benchmark skill link does not point at the packaged skill",
    )
    assert(existsSync(join(linkedSkillRoot, "SKILL.md")), "linked vllm-benchmark skill has no SKILL.md")
    ...
    const remove = ["exec", "--offline", "--", "vllm-benchmark-configure", "remove"]
    ...
    assert(!parsedAfterRemove.agent?.["vllm-benchmark"], "remove left a direct config.agent entry")
    assert(
      Array.isArray(parsedAfterRemove.plugin) && !parsedAfterRemove.plugin.includes(expectedPluginSpec),
      "remove left the vllm-benchmark plugin registration",
    )
    assert(!existsSync(skillLink), "remove left the vllm-benchmark skill link")
    ...
    const report = {
      ...
      packageName,
      networkIsolation: process.env.VLLM_BENCHMARK_NETWORK_ISOLATION || "none",
      npmInstallPreservedConfig: true,
      setupIdempotent: true,
      setupPreservedConfig: true,
      installedRunnerPassed: true,
