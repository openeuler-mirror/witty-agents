...
function verifyVariant(variant, basePackage, packageStyle, arch) {
  ...
  if (variant === "online") {
    ...
    assert(report.pythonDependencies === "download-on-install", "online: python dependencies should download on install")
  } else {
    ...
    assert(report.pythonDependencies === "bundled-wheels", "offline: python dependencies were not bundled as wheels")
    assert(Number.isInteger(report.pythonWheelCount) && report.pythonWheelCount > 0, "offline: python wheel count is missing or zero")
    assert(typeof report.wheelPlatform?.machine === "string" && report.wheelPlatform.machine.length > 0, "offline: wheel platform machine is missing")
