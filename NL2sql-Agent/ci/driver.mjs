...
function verifyInstall(variant) {
  ...
  if (variant === "online") {
    run(process.execPath, args, { env: { NL2SQL_NETWORK_ISOLATION: "none" } })
    return
  }
  ...
  if (!unshareArgs) {
    ...
    run(process.execPath, args, { env: { NL2SQL_NETWORK_ISOLATION: "not-enforced" } })
    return
  }
  run("unshare", [
    ...unshareArgs,
    "sh", "-c",
    "ip link set lo up && export NL2SQL_NETWORK_ISOLATION=\"$1\" && shift && exec \"$@\"",
...
function main() {
  ...
  if (options.phase === "prepare") prepare(options)
  else if (options.phase === "prepare-assets") console.log("[nl2sql-ci] no external assets to prepare")
    ...
    env: { NL2SQL_TEST_VARIANTS: options.variants.join(",") },
  })
...
try {
  ...
  console.error(`[nl2sql-ci] ${error.message}`)
  process.exit(1)
}