...
6. **Root cause validation** — generate `root_cause_analysis.json` after verifying call trace completeness, module consistency, and source-code mapping. If incomplete, run a second round. This section should be **LLM summarization** based on all collected evidence. Produce a single `root_cause` **natural-language paragraph** (连贯叙述, 不分点/不用 markdown 小标题) that naturally weaves together: first a one-sentence crash phenomenon; then the root cause branched on `diagnosis_repair_result` — if `internal_kernel_result` matched → derive from local case `root_cause`; if only `community_kernel_result` matched → derive from community case; if neither matched → **must** invoke the git skill to query related commit/issue as reference (record the query in this stage's `tool_calls`), then derive root cause from results, or mark as 「推测」 if git skill yields nothing; along the way mention probable triggering scenarios (e.g. load, concurrency/race, memory-pressure); finally, only when a match exists, append knowledge-base retrieval info (`knowledge_id`, source internal/community, `match_score` level) — omit this part when no match. For `solution` (always required, also a **natural-language paragraph**, 不分点/不用小标题): tier by evidence strength — (a) if `internal_kernel_result`/`community_kernel_result` matched → adopt the case's `solution` as the fix, narrate in prose; (b) if no match but git skill returned commit/issue → give a reference fix suggestion based on the community commit, marked 「参考社区 commit xxx」, narrate in prose; (c) if git skill also yields nothing → write a 「处置建议」 (handling advice) rather than a fix: narrate temporary mitigations, information-gathering suggestions, and next-step investigation directions (e.g. deeper vmcore analysis, contact kernel team, upgrade to LTS for verification) in a flowing paragraph, do not fabricate fix code; keep `diagnosis_repair_result` arrays empty, no fabricated cases.
...
11. **Generate the user-facing core report** — run `scripts/generate_core_report.py` to produce `core_report.json` alongside `report.json`. The full `report.json` is the technical archive (workflow_trace + raw case JSON); `core_report.json` is the compact operator-facing view (crash time, host, bug summary, call trace, root cause, solution, match info) — roughly 12% the size of the full report, generated by pure field-mapping (no LLM):

    ```bash
    .venvs/crash-report-generator/bin/python skills/crash-report-generator/scripts/generate_core_report.py \
        --report report.json \
        --output core_report.json \
        --validate
    ```
