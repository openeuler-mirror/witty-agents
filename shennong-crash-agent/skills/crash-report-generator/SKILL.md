...
6. **Root cause validation** — generate `root_cause_analysis.json` after verifying call trace completeness, module consistency, and source-code mapping. If incomplete, run a second round. This section should be **LLM summarization** based on all collected evidence. For `solution`: if a matching case exists in `diagnosis_repair_result`, adopt its `solution`; if no case matched (`query_knowledge` / `query_cases` / `query_community_cases` all return empty), still write a speculative solution/direction into `solution` (clearly marked as “推测”) that describes the probable triggering scenarios (e.g. load, concurrency/race, memory-pressure conditions) and note it in `analysis_notes`; keep `diagnosis_repair_result` arrays empty, no fabricated cases.
