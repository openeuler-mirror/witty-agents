...
5. **宕机特征章只留崩溃直接事实**：异常特征表仅含异常类型/故障地址/RIP/现场上下文；「原始崩溃日志」= 崩溃本身（`raw_crash_log` 完整原文）。「主机与运行环境」为技术附件第一卡；重复可疑日志、其它异常等背景信号也收在技术附件「日志特征明细」（三类全量 + 相关性徽标 + 逐行 desc），避免低参考价值内容干扰主线阅读。
6. **原始崩溃日志优先完整原文 + 逐行悬停解读**：「宕机特征」章的「原始崩溃日志」优先渲染 `crash_feature_info.raw_crash_log`（完整连续崩溃段）；无则退化为 `related_errors[].line` 按行号拼装摘录；两者都没有才单独展示「崩溃调用栈」（`call_trace_text`，不冒充原始日志）。不再设独立的「如何阅读这段日志」指南区——日志按**连续同类行合并为语义块**（红=崩溃类型、琥珀=ESR 异常综合征/故障地址、橙=崩溃指令定位 pstate/pc/lr/sp、蓝=寄存器、绿=调用栈、紫=崩溃机器码 Code、蓝灰=kdump 捕获、灰=现场环境），每块带左侧色条与浅色底纹，悬停/点击时**整块高亮**并显示块级解读（tooltip，点击钉住、点空白取消），解读直接绑定在日志块上；日志区顶部有**色块图例**（色片+类别名，非纯文字）。
7. **判断依据为连续追问链**：第 4 章「为什么得出这个结论」的每条依据按 1/2/3… 编号呈现，并用 `connect` 串联为递进链（上一条结论 → 引出下一条要回答的问题），引言注明「从崩溃现场出发，一步步追溯根因并交叉验证」；编号仅是链条顺序，不是并列角度。
...
12. **推理步内联原文证据块（snippets）**：`reasoning[].snippets` 渲染为折叠卡（默认收起，短日志 inline 直出），逐行+行号+琥珀高亮+行尾 ◀ 注释（现场日志/调用栈/反汇编的关键行——崩溃行/故障指令行/关键调用帧——必配 `ann`，源码/内存取证照旧）；`segs` 多段子框每段独立文件行号/ref；有 snippets 的步骤自动隐藏 evidence 一行式清单（无 snippets 的自动转为块状事实块）。
...
6. **根因验证** — 在核验调用栈完整性、模块一致性、源码映射后生成 `root_cause_analysis.json`；若不完整则再跑一轮。该节应由 **LLM 综合所有证据总结**，产出与 report.html 第 1/3/4 章一致的字段：
   ...
   - **`deep.evidence[]`**（逐条核心依据）：每条使用对象 `{title, summary, level, caveat, reasoning[], connect}`（`title` 必填设问式、`connect` 串联、`level` 证据级别徽标、`caveat` 未决边界，规则见 generate-report.md）；`reasoning[]` 是该依据自己的证据推理链，每步填写 `step`/`detail`/`evidence`（`evidence` 用类别前缀结构化清单，禁堆工具原始输出），**建议以 `step="推导与结论"` 步收尾**（可复核推导 → `conclusion`，该步可不配 snippet）。HTML 把 `level` 渲染为结论行徽标（实测=绿/推断=琥珀/未定=灰），`caveat` 渲染为「边界」行，并为每条依据单独提供展开面板。
...
3. 进行**质量软检查（WARN 级，不影响退出码）**——以下形态会在 stderr 打印 `[WARN]`，交付前应逐条修正到零 WARN：
   ...
   - `现场日志`/`反汇编`/`调用栈` 的 snippet 有 `hl` 但无 `ann`——关键行补行尾 `◀` 注释；
