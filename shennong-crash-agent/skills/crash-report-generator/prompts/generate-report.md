...
12. **Generate the user-facing core report** — run `generate_core_report.py` to produce `core_report.json` alongside `report.json`. The full `report.json` is the technical archive (workflow_trace + raw case JSON); `core_report.json` is the compact operator-facing view (crash time, host, bug summary, call trace, root cause, solution, match info) — roughly 12% the size of the full report, generated by pure field-mapping (no LLM):
