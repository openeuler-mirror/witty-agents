...
6. **`root_cause_analysis.json`**（LLM 综合多源证据总结），字段与 report.html 第 1/3/4 章一致。**`conclusion` / `event_scene` / `propagation_chain` / `reasoning_flow` / `deep` 均为必填（schema 强校验），缺一不可**：
   ...
   - **`deep.flow`**（故障流程梳理，必填）：用箭头链只描述触发流程，不含根因判定与处置方向。
    ...
    - **`deep.evidence[]`**（判断依据，必填）：按**自然分析流程逐步递进**，从崩溃现象出发，每一步回答一个「为什么 / 怎么知道」的问题，用 `connect` 把上一步结论自然引到下一步问题；**步数不限、标题用设问式**——`title` 写成**设问句**（含「为什么 / 怎么 / 在哪」等疑问词，如「宕机栈分析：崩在哪里、为什么崩」「空指针从哪来：这个空指针是怎么传进来的」「为什么空值没被拦住」），**禁止**结论式平铺标题（如「崩溃现场：xxx」「根因定位：xxx」「反汇编验证：xxx」）；不要为了套固定框架而跳过必要的中间环节。每条 `{title,summary,reasoning[],connect}`，第 1 条不填 `connect`，其余每条**必填** `connect`（引用上一条结论 + 引出下一条要回答的问题）。
