...
5. **`crash_feature_info.json`**：
   ...
   - **`raw_crash_log`**（**必填**，从原始日志文件逐字提取）：**完整连续的崩溃日志段**——从崩溃首行（如 `Unable to handle kernel ...` / `BUG: ...` / `Kernel panic ...`）起，到 `Code: ...` 机器码行止（x86 无 Code 行则到 Call Trace 结束，可再含紧随的 SMP stopping/kdump 启动行）。必须用 `sed -n '<起>,<止>p' <dmesg文件>` 原样复制，**不得截断寄存器行、不得省略 ESR/pstate/lr/sp/Call trace 任何一行、不得改写时间戳**。HTML「原始崩溃日志」区优先渲染本字段：**默认收起、展开后限高滚动**，只对关键行（崩溃首行/BUG、故障地址、RIP/pc/lr、Call Trace、`<IRQ>`/`</IRQ>`、Code、panic）做琥珀高亮 + 行尾 ◀ 注释，主打一目了然；缺失本字段会退化为 related_errors 摘录拼装或 call_trace_text。
    - **`raw_crash_log_ann`**（关键行注释，与 `raw_crash_log` 配套、强烈建议）：对象 `{行号: 注释}`（行号为 `raw_crash_log` 内 1-based 行号），只标关键行 3~8 条（崩溃类型/BUG 行、故障地址/CR2、RIP/pc、lr、Call Trace、`<IRQ>`/`</IRQ>`、Code、panic）。**注释必须一眼看懂**：① 崩溃函数是干什么的（结合调用链/子系统职责，如「dma_direct_unmap_sg——完成路径里解除 DMA 映射、逐段遍历散列表的函数」）；② 中断里正在发生什么（谁给谁做什么收尾/处理）；③ 数值与偏移的来由（如「0x18 = scatterlist.dma_length 字段偏移」「R14=0、R13=13：表到头了却按 32 段继续」）。**禁止只写「崩溃类型行」「崩溃指令位置」「故障指令机器码」这类标签**，也不要复述行内容。正例：`"崩溃点：dma_direct_unmap_sg——完成路径里负责解除 DMA 映射的函数，此刻正读 sg->dma_length"`。缺失时模板按行类型自动兜底（通用文案）。
...
6. **`root_cause_analysis.json`**（LLM 综合多源证据总结），字段与 report.html 第 1/3/4 章一致。**`conclusion` / `event_scene` / `propagation_chain` / `reasoning_flow` / `deep` 均为必填（schema 强校验），缺一不可**：
   ...
   - **`deep.flow`**（故障流程梳理，必填）：用箭头链只描述触发流程，不含根因判定与处置方向。
    ...
    - **`deep.evidence[]`**（判断依据，必填）：按**自然分析流程逐步递进**，从崩溃现象出发，每一步回答一个「为什么 / 怎么知道」的问题，用 `connect` 把上一步结论自然引到下一步问题；**步数不限、标题用设问式**——`title` 写成**设问句**（含「为什么 / 怎么 / 在哪」等疑问词，如「宕机栈分析：崩在哪里、为什么崩」「空指针从哪来：这个空指针是怎么传进来的」「为什么空值没被拦住」），**禁止**结论式平铺标题（如「崩溃现场：xxx」「根因定位：xxx」「反汇编验证：xxx」）；不要为了套固定框架而跳过必要的中间环节。每条 `{title,summary,level,caveat,reasoning[],connect}`：第 1 条不填 `connect`，其余每条**必填** `connect`（引用上一条结论 + 引出下一条要回答的问题）；`level` **必填**（证据级别徽标，如「现场实测」「实测+推断」「推断」「未定」，按实际证据强度标注、不得拔高）；`caveat` 写该条的未决边界（仍是推断的部分 + 还缺什么证据），不写进 `summary` 正文。
