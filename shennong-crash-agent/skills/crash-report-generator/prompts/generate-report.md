...
6. **`root_cause_analysis.json`**（LLM 综合多源证据总结），字段与 report.html 第 1/3/4 章一致：
   ...
   - **`event_scene`**（事件时序图，三泳道：进程 / 内核 / 硬件）：
     - `participants`（对象泳道，3–5 个）：`id`（简短英文，如 `user`/`kern`/`hw`）、`name`（具体到进程名+PID、CPU 序号、硬件名）、`type` 三选一 `process`（进程/用户态业务线程）/ `kernel`（内核/CPU）/ `hardware`（硬件）、`init`（初始状态）、`tip`（一句话说明）。
     ...
     - `events`：每条 `{m, from, to, kind, title, val, t, dt_ms, full, g}`：
       ...
       - `from`/`to`：用 participant `id` 表示**泳道间箭头**（如 `user`→`kern`、`kern`→`hw`），每个框都要有来源/去向；
