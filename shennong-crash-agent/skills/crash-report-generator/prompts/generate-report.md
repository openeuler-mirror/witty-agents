...
5. **`crash_feature_info.json`**：
   ...
   - **推理步内联原文证据块（snippets，每步必配）**：`deep.evidence[].reasoning[]` 的**每一个步骤都至少要配 1 个 snippet**，禁止只写 `evidence` 不配 snippet。按材料类型选择 kind 并填齐字段：
     ...
     - **证据块 `ann` 口径（第 4 章，逐行注释）**：每行**≤35 字**，用动词句写清「这行在做什么 + 得到 / 证明什么」，术语与数值保留便于核对（如「实测段数=32——但表实际只有 13 段」「空闲链表头指向 M——下次分配从它开始」）；**避免名词堆叠**（如「取 iod->nents（偏移 0x184）」「由 cpu_slab 计算出的 per-CPU 分配器地址」）与**同义反复**（如「命中请求 tag=594」「分配函数 mempool_kmalloc」）。第 2 章 `raw_crash_log_ann` 保持「初步定位」口径（15~40 字），与第 4 章不重复。
