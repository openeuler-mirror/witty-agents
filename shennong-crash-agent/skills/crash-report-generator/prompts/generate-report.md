...
- **LLM 补齐的分析字段**：
  ...
  - `history`：对象 `{first_seen, last_seen, cases[]}`——`first_seen`/`last_seen` 取原生 `first_seen`/`last_seen`（无则用同簇案例日期），`cases[]` 列出各次崩溃点；
...
5. **`crash_feature_info.json`**：
   ...
   - **`raw_crash_log`**（**必填**，从原始日志文件逐字提取）：**完整连续的崩溃日志段**——从崩溃首行（如 `Unable to handle kernel ...` / `BUG: ...` / `Kernel panic ...`）起，到 `Code: ...` 机器码行止（x86 无 Code 行则到 Call Trace 结束，可再含紧随的 SMP stopping/kdump 启动行）。必须用 `sed -n '<起>,<止>p' <dmesg文件>` 原样复制，**不得截断寄存器行、不得省略 ESR/pstate/lr/sp/Call trace 任何一行、不得改写时间戳**。HTML「原始崩溃日志」区优先渲染本字段：**默认收起、展开后限高滚动**，只对关键行（崩溃首行/BUG、故障地址、RIP/pc/lr、Call Trace、`<IRQ>`/`</IRQ>`、Code、panic）做琥珀高亮 + 行尾 ◀ 注释，主打一目了然；缺失本字段会退化为 related_errors 摘录拼装或 call_trace_text。
    - **`raw_crash_log_ann`**（关键行注释，与 `raw_crash_log` 配套、强烈建议）：对象 `{行号: 注释}`（行号为 `raw_crash_log` 内 1-based 行号），只标关键行 3~8 条（崩溃类型/BUG、故障地址/CR2、RIP/pc、lr、Call Trace、`<IRQ>`/`</IRQ>`、Code、panic）。**注释只做「初步定位」**（约 15~40 字）：说明「这行是什么 + 为何值得看一眼」，可带一点初步指向，让读者乍一眼知道大概是什么问题；**不展开因果与机制、不解释数值来由**——那些属于第 4 章 `deep.evidence` 的 ◀ 注释，**禁止与第 4 章注释重复或近似**，也不要只写「崩溃类型行」这类空标签。正例：`"崩溃点：dma_direct_unmap_sg——正在解除 DMA 映射时读段长度"`；反例（过长/重复第 4 章）：`"…0x18 是 scatterlist.dma_length 字段偏移，因为请求 594 的散列表被请求 612 重建为 13 段…"`。缺失时模板按行类型自动兜底（通用文案）。
   ...
   - **推理步内联原文证据块（snippets，每步必配）**：`deep.evidence[].reasoning[]` 的**每一个步骤都至少要配 1 个 snippet**，禁止只写 `evidence` 不配 snippet。按材料类型选择 kind 并填齐字段：
     ...
     |---|---|---|---|
     | 崩溃日志/panic | 现场日志 | dmesg 原文段（逐字） | `loc`(文件:行号) + `hl` + `ann` |
     | 调用栈 | 调用栈 | Call Trace 原文段 | `loc` + `hl` + `ann` |
     | 汇编/机器码 | 反汇编 | 反汇编或 Code 窗口原文 | `loc` + `hl` + `ann` |
     | 源码对照 | 源码 | 真实源码原文（**不带行号前缀**；连续段设 `first_line`，多区间/跳段拆 `segs`） | `loc` + `first_line` + `hl` + `ann` + `ref` |
     | 内存取证 | 内存取证 | crash 会话版式实测值（逐字） | `loc` + `hl` + `ann` |
     | 推断/定性 | 定性 | 推理依据逐条（每条一行，**不写行号**） | `loc`(如「推断整理」) + `hl`(可选) + `ann`(整段小结) |
     | 版本对照 | 上游对照 | 版本对照事实（每条一行，不写行号） | `loc` + `hl`(可选) + `ann`(小结) |
     | 检索结论 | 检索记录 | 检索事实（每条一行，不写行号） | `loc` + `hl`(可选) + `ann`(小结) |
     补充：原文类块（现场日志/调用栈/反汇编/源码/内存取证）`ann` 用「行号→注释文本」解释关键行实测值/语义——**现场日志、调用栈、反汇编三类块的关键行（崩溃行 / 故障指令行 / 关键调用帧）必配 `ann`**，源码/内存取证照旧，其余行可选；**`ann`/`hl` 的键是内容第 N 行**（1 起，命令行/`...` 行照数；模板对有 `first_line` 的块也兼容绝对文件行号写法，但统一推荐相对键）。非原文类块（定性/上游对照/版本对照/检索记录/事实记录）渲染为**结论清单卡**（无行号、非等宽）：每条一行、行首可带 `✓/✗/≈/-` 语义标记，`ann` 收成卡底「小结/结论」条（不再逐行挂 ◀），示例：`{"kind":"检索记录","content":"内部知识库命中 1 条同位置旧案例（匹配度中等）\n社区/上游 0 条","ann":{"1":"三路均无现成修复——不足采用"}}`。短日志（≤4 行）设 `inline: true` 始终可见；`ref` 链附件 local_source 时该段显示「完整摘录」。
     - **源码块行号规范（first_line / segs）**：源码块 content 只放源码原文，**禁止保留 `422 …` 这类行号前缀**（左栏由模板按 `first_line` 渲染真实文件行号）；连续摘录设 `first_line`，跳段/多文件必须拆 `segs`（每段独立 `first_line` + `hl` + `ann`），**禁止用 `...` 行代替跳段**。`hl`/`ann` 的键一律按**内容第 N 行**数（示例：`first_line=416`、内容第 7 行是 `for_each_sg(...)`，则 `hl:[7]`、`ann:{"7":"遍历循环……"}`）。多段示例：`{"kind":"源码","segs":[{"loc":"drivers/nvme/host/pci.c:588-609","first_line":588,"content":"…"},{"loc":"drivers/nvme/host/pci.c:839-842","first_line":839,"content":"…"}]}`。生成后必须逐条自检：键指向的那行文本与注释语义是否一致；无法确定位置的行宁可不标。**注释锚点自检（源码块）**：优先用行内注释（注释与行天然绑定）；若使用 `ann` 键，则把注释文本里的关键标识（函数名/字段/宏/数值，如 `task->mm`、`IS_ERR`、`mm_access`、`0xb0`）逐词在 `content` 里定位，确认其出现行号＝`ann` 键；若只出现在相邻行，即为错位，必须把注释移到对应行（validator 会对源码块输出「注释疑似错位」WARN）。
     - **注释优先写法：行内注释（推荐）**：注释直接写在内容行尾——`<代码行/日志行>   ◀ 注释文本`；模板自动解析并**高亮该行**，无需再填 `hl`/`ann`。约束：整行只能有一个 `◀`、前后都有非空文本、注释文本非空；同一行不要同时给行内注释与 `ann`（行内优先）。清单类块（定性/检索记录等）不支持行内注释，其小结仍写 `ann`；**第 2 章 `raw_crash_log` 必须逐字原文 + `raw_crash_log_ann`**，不得掺入 `◀`。
     - **证据块 `ann` 口径（第 4 章，逐行注释；兼容写法）**：每行**≤35 字**，用动词句写清「**为什么看它（由上一问引出）+ 这行在做什么 + 得到 / 证明什么**」，让读者明白「为什么盯住这个指针/函数/偏移」；术语与数值保留便于核对（如「实测段数=32——但表实际只有 13 段」「空闲链表头指向 M——下次分配从它开始」）；**避免名词堆叠**（如「取 iod->nents（偏移 0x184）」「由 cpu_slab 计算出的 per-CPU 分配器地址」）与**同义反复**（如「命中请求 tag=594」「分配函数 mempool_kmalloc」）。第 2 章 `raw_crash_log_ann` 保持「初步定位」口径（15~40 字），与第 4 章不重复。
     - **crash 类证据块（内存取证 / 反汇编 / 调用栈）口径**：`content` 必须是**原始输出（逐字）**——内存取证/反汇编首行为 `crash> <命令>`，调用栈可用 `crash> bt` 或 dmesg `Call Trace:` 原文（首行即为原文首行）；**禁止把输出改写成摘要**、**禁止自造「项N / 第N段」等编号行**（行号由模板统一提供）。`title` **必填**（一句话说明这块看的是什么，如「crash rd：散列表缓冲区第 0~16 项」）；`loc` 只写范围/对象（如「散列表缓冲区 第 11~14 项」「请求 A 附属结构」），**不要重复命令**（命令已在内容首行）。
   ...
   - **路径脱敏**：报告数据中**禁止出现绝对路径**（/opt/、/home/、/tmp/、/root/ 等）——crash 现场路径仅保留文件名（如 vmcore-dmesg.txt），源码树用"本地源码树（版本号）"，知识库用"本地知识库"。
     ...
     - `hl`（关键行高亮，**强烈建议填写**）：excerpt 内 1-based 行号数组，标注**崩溃行 / 关键调用行 / 补丁落点行**（如 for_each_sg 循环条件行、`dma_unmap_sg(...iod->nents...)` 调用行、`return NULL` 行）；HTML 逐行渲染并对命中行高亮（强调底色+行号着色），读者可一眼锁定关键代码；行号必须在 excerpt 行数范围内。
...
6. **`root_cause_analysis.json`**（LLM 综合多源证据总结），字段与 report.html 第 1/3/4 章一致。**`conclusion` / `event_scene` / `propagation_chain` / `reasoning_flow` / `deep` 均为必填（schema 强校验），缺一不可**：
   ...
   - **`trigger_scenario`**（易触发场景，一句话）：写成「在某场景下，某主体做某操作就容易触发」，不写具体业务名；**禁止出现 CPU 编号、PID、进程名、主机名、请求编号、时间戳等现场细节**（那是第 2 章现场初印象的职责），要写可推广的环境/负载条件，必要时给出前提条件。
     - 特化：`在类高频迁移业务的线程频繁睡眠/唤醒（如 nanosleep 让出 CPU）的场景下，内核调度器挑选下一任务时即可能触发。`
     - 特化（含前提）：`在高并发写回与强制刷盘场景下，若内存分配器空闲链已出现异常（如自环），多条在途请求共用同一 SG 表内存时，完成中断解除映射即可能触发。`
   ...
   - **`event_scene`**（事件时序图，**必填**，采用“固定骨架 + 有限推断”，避免同一日志生成互相矛盾的时序）：
     ...
     - `participants`（对象泳道，可多个）：`id`（简短英文，如 `proc_a`/`cpu92`/`nic0`）、`name`（进程写「进程名+PID」，cpu 写「CPU 编号 · 型号」，hardware 写「设备名/型号」）、`type` 三选一 `process`（进程/用户态业务线程）/ `cpu`（CPU 核）/ `hardware`（外部硬件）、`init`（初始状态）、`tip`（一句话说明，供 hover 展示）。
      - `anchor`：崩溃时刻锚点，**必须是崩溃发生（panic/Oops/宕机指令执行）的精确时间**，**ISO 8601 带毫秒**（如 `2026-07-08T06:20:00.000`），供查看器按 `dt_ms` 反推每个事件的「时分秒」；**禁止写任何非时间字符串**（如 `evt-sched`、`T+0`、`anchor` 这类占位符），也禁止把 anchor 当成最早触发动作的时间。
      ...
      - `events`：每条 `{m, from, to, kind, t, val, dt_ms, title, full, g}`，字段语义如下（**严格按此填写，禁止互换**）：
        - `t`：**事件描述正文**（一句话，直接写「这一步发生了什么」，显示在泳道框的步骤标签里），如 `nanosleep() 系统调用陷入内核（SYSCALLNO=0x65）`；**⚠️ `t` 不是时间、不是序号，禁止填「T+0/T0/T1/T2」或任何时间戳**（时间由 `dt_ms` 单独表达）；`title` 才是可选的加粗短标题，两者**不要互换**；
   ...
   - **`deep.flow`**（故障流程梳理，必填）：用箭头链只描述触发流程，不含根因判定与处置方向。
    ...
    - **`deep.evidence[]`**（判断依据，必填）：按**自然分析流程逐步递进**，从崩溃现象出发，每一步回答一个「为什么 / 怎么知道」的问题，用 `connect` 把上一步结论自然引到下一步问题；**步数不限、标题用设问式**——`title` 写成**设问句**（含「为什么 / 怎么 / 在哪」等疑问词，如「宕机栈分析：崩在哪里、为什么崩」「空指针从哪来：这个空指针是怎么传进来的」「为什么空值没被拦住」），**禁止**结论式平铺标题（如「崩溃现场：xxx」「根因定位：xxx」「反汇编验证：xxx」）；不要为了套固定框架而跳过必要的中间环节。每条 `{title,short,summary,level,reasoning[],connect}`：第 1 条不填 `connect`，其余每条**必填** `connect`（引用上一条结论 + 引出下一条要回答的问题）；`short` **必填**（4~10 字索引导航标签，如「定位空指针」「追查空值来源」「确认影响范围」，用于第 4 章顶部 `1./2./3.` 索引，缺失时模板回退标题截断）；`level` **必填**（证据级别徽标，如「现场实测」「实测+推断」「推断」「未定」，按实际证据强度标注、不得拔高）；未决/不确定信息并入该条结论或推导末尾一句表达，**不再单列「证据边界」**。
      - **必须覆盖的环节（缺一不可，可再加步）**：① 崩在哪（崩溃点还原）→ ② **这个异常值/空指针是从哪传进来的**（用 LR/反汇编/参数追溯，说明是谁把它传给了崩溃函数）→ ③ 为什么会产生这个异常（结构体/计数器取证）→ ④ **影响范围量化**（有多少对象/请求/CPU 受影响——有取证就写数，如「至少 19 条活跃请求共用同一块内存」；取证不到就写「范围未测」）→ ⑤ **继续追到更上游根因**（不要停在直接原因，追问「是什么机制/操作造成了它」，直至可行动的根因）→ ⑥ 为什么没有防御拦住（源码/反汇编对照）→ ⑦ **未决边界**（哪些结论仍是推断、首次破坏者/触发源是否确定、还需什么证据补齐）→ ⑧ 社区/上游交叉验证。
      - **禁止跳步**：例如「崩在哪」直接跳「为什么空」、漏掉「这个空值是谁传进来的」这一环，会导致前后脱节、读者看不懂 se=NULL 从哪来。
      - **证伪纪律**：被证据推翻的说法/假设必须写明「X 不成立，依据 Y」（如「该请求从未下发」被提交队列命令证伪），不要只留正确的推论、把证伪过程藏起来。
      ...
      - **建议以「推导与结论」步收尾**：推荐每条 `reasoning[]` 以 `step="推导与结论"` 结尾——`detail` 写**可复核的推导过程**（数值代入/算式/源码逻辑，如「把 R14=0 代入 `mov 0x18(%r14)` → 实际访问 0x18」「0x180÷0x20=12 → 结束标记落在第 13 项」），`conclusion` 给该条依据的结论；推导步若确无可贴的原文，可不配 snippet（其余步骤仍须至少 1 个 snippet）。
      - **正例（链条骨架，照此推进）**：
        ```json
        {"title":"崩在哪个函数，空的是哪个指针？","level":"现场实测","summary":"内核在 dma_direct_unmap_sg 中读取了空的 scatterlist 指针，触发宕机。","reasoning":[
          {"step":"崩在哪一行","detail":"故障指令读「散列表指针+0x18」，该指针为空，访问 0x18 直接触发缺页。","snippets":[{"kind":"现场日志","loc":"vmcore-dmesg.txt:3226","hl":[1],"ann":{"1":"0x18 是 dma_length 字段偏移，不是随机值"},"content":"BUG: kernel NULL pointer dereference, address: 0000000000000018"}]},
          {"step":"推导与结论","detail":"把 R14=0 代入 mov 0x18(%r14) → 实际访问地址 0x18；对照结构体布局，0x18 正是 dma_length 的偏移。","conclusion":"内核在 dma_direct_unmap_sg 中读取了空 SG 指针的 dma_length 字段。"}
        ]}
        ```
      - **内容顺读五铁律（防「跳跃 / 看不懂」——历史实证）**：
        1. **对象先解释**：每个非通用术语/对象（段表（SG 表）、请求、请求档案（iod）、完成回调、内存池、命令号等）**首次出现必须就地用一句人话解释**（是什么、在本次故障里干什么），后文复用同一称呼；**第 1 环先立最小对象模型**（一次 I/O 请求 → 请求附属的段表 → 完成后段表回收入池 → 内存被下一个请求复用），后续环不得引入未解释的新对象。
           - 坏例：「被打断上下文的散列表指针与完成回调结构里记录的散列表指针逐字相同」——读者不知道「完成回调结构」是什么。
           - 好例：「完成回调手上有一份请求档案（记录该请求的段表指针与段数）；这份档案里的段表指针，与被中断线程正在用的段表指针是同一个地址。」
        2. **环间复述**：`connect` 必须写成「上一环结论（人话一句）+ 由此追问（人话一句）」，**禁止只写疑问句**，也禁止引入上一环未出现过的概念。
           - 坏例：「第一次完成合法，那第二次完成处理是谁发起的？」
           - 好例：「第四环确认了『还表→取表』的复用是正常机制；那问题只剩一个——资源都已回收，怎么还会有第二次『完成』用到这张表？」
        3. **推导列前提**：`推导与结论` 步按「已知①…；已知②…；→ 结论」逐条列（每条只前进一步，人话为主），推断显式标「（推断）」；结论不得引入推导里没出现过的新事实。
           - 坏例：「完成通知唯一生产者=控制器固件」。
           - 好例：「已知①能产生完成的只有盘固件上报与内核超时/复位/轮询三条补救通道（推断）；已知②45 天日志无超时/复位记录、其余核无轮询动作；→ 三条通道全部排除，这条凭空出现的完成只能来自固件（推断）。」
        4. **术语/编号一致**：同一对象全文一个叫法（如统一「段表（SG 表）」「完成回调」「命令号」）；0 起与 1 起首次出现必须注明并全文统一（如「下标 12（从 0 数）＝第 13 项」）。
        5. **量化有据**：「毫秒 / 天 / 次数 / 条数」等量化必须来自证据，取证不到的标注（推断）；不得为了生动编造量级。
        - **自检两条**：① 单环自洽——只看该环 `title + reasoning + 推导 + summary` 能否独立成立；② 全链通读——只看各环 `connect` 能否把所有依据连成一个完整故事。不满足就改到满足为止。
...
7. **`diagnosis_repair_result.json`**（脚本/工具生成）：复用第 6 步开头的匹配工具响应，逐字节复制；社区案例走双通道（rag_core / 本地 grep+find，取 Top 3-4）。**未命中的分类写空数组 `[]`，严禁编造/幻想条目**（内部案例、社区邮件、会议纪要、Bugzilla、上游 commit 各自独立：有命中才填，没有就空）。**⚠️ 报告不得引用既往生成的报告**：`internal_kernel_result` 只填 `query_knowledge`（RAG core）真实返回的内部案例；RAG 未配置或未命中时置空（总览显示「未匹配到内部案例」）。严禁把 `report_*.json` / `crash-report_*.html` 等既往报告当作案例、证据或结论依据——正文、结论、章节与附录一律不得出现对既往报告的引用；**也不得读取既往报告文件**（输出目录/现场目录中发现历史报告一律忽略，最多在 workflow_trace 记为「已忽略」）。
