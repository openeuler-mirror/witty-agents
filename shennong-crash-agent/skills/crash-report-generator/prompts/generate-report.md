...
5. **`crash_feature_info.json`**：
   ...
   - **推理步内联原文证据块（snippets，每步必配）**：`deep.evidence[].reasoning[]` 的**每一个步骤都至少要配 1 个 snippet**，禁止只写 `evidence` 不配 snippet。按材料类型选择 kind 并填齐字段：
     ...
     - **源码块行号规范（first_line / segs）**：源码块 content 只放源码原文，**禁止保留 `422 …` 这类行号前缀**（左栏由模板按 `first_line` 渲染真实文件行号）；连续摘录设 `first_line`，跳段/多文件必须拆 `segs`（每段独立 `first_line` + `hl` + `ann`），**禁止用 `...` 行代替跳段**。`hl`/`ann` 的键一律按**内容第 N 行**数（示例：`first_line=416`、内容第 7 行是 `for_each_sg(...)`，则 `hl:[7]`、`ann:{"7":"遍历循环……"}`）。多段示例：`{"kind":"源码","segs":[{"loc":"drivers/nvme/host/pci.c:588-609","first_line":588,"content":"…"},{"loc":"drivers/nvme/host/pci.c:839-842","first_line":839,"content":"…"}]}`。生成后必须逐条自检：键指向的那行文本与注释语义是否一致；无法确定位置的行宁可不标。**注释锚点自检（源码块）**：把 `ann` 文本里的关键标识（函数名/字段/宏/数值，如 `task->mm`、`IS_ERR`、`mm_access`、`0xb0`）逐词在 `content` 里定位，确认其出现行号＝`ann` 键；若只出现在相邻行，即为错位，必须把注释移到对应行（validator 会对源码块输出「注释疑似错位」WARN）。
     - **证据块 `ann` 口径（第 4 章，逐行注释）**：每行**≤35 字**，用动词句写清「**为什么看它（由上一问引出）+ 这行在做什么 + 得到 / 证明什么**」，让读者明白「为什么盯住这个指针/函数/偏移」；术语与数值保留便于核对（如「实测段数=32——但表实际只有 13 段」「空闲链表头指向 M——下次分配从它开始」）；**避免名词堆叠**（如「取 iod->nents（偏移 0x184）」「由 cpu_slab 计算出的 per-CPU 分配器地址」）与**同义反复**（如「命中请求 tag=594」「分配函数 mempool_kmalloc」）。第 2 章 `raw_crash_log_ann` 保持「初步定位」口径（15~40 字），与第 4 章不重复。
