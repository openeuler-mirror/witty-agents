...
6. \`root_cause_analysis.json\`：**LLM 基于上下文总结生成**。在已有多源证据（基线、崩溃特征、内部/社区案例、日志检测、源码分析、在线 commit/patch/邮件）基础上，产出与 report.html 第 1/3/4 章一致的字段：
   ...
   - 口径规则：① \`crash_feature_info.stack_impression\` 按本次宕机栈写一句话现场初印象（禁止固定话术）；② 上游三路检索均无 confirmed 补丁时，\`fixed_brief/brief\` 直写「上游无对应补丁，需自研适配或提供更多信息诊断」；③ 有本地源码且知识库无案例时，**必须**基于源码出 patch_list（diff 基于真实源码）并在 \`claim_tag\` 标注补丁来源，diff 落点文件列入 \`local_source.refs\`；④ 临时规避空方案的「根治途径」表述与 \`standard_solution.type\` 同源；⑤ `raw_crash_log` 必须配套 `raw_crash_log_ann`（BUG/故障地址/RIP/Call Trace/Code/panic 等关键行 3~6 条 ◀ 注释）；⑥ 第 1 章「历史案例」格只取内部案例库，未命中写「未匹配到内部案例」。
