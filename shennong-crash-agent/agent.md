...
5. **原生 JSON 逐值直通（内部案例有分析字段例外）**：\`diagnosis_repair_result.internal_kernel_result\` 的**核心字段**（\`knowledge_id\`/\`fingerprints\`/\`bug_summary\`/\`root_cause\`/\`solution\`/\`hotpatch\`/\`kernel_versions\`/\`case_count\`/\`match_level\` 等）必须逐字段、逐值复制 \`crash-feature-matcher\` 返回的原生 JSON 对象，禁止改写 value、禁止重新总结、禁止字段名映射、禁止数值归一化或类型转换；在此基础上由 LLM 补齐分析字段（\`id\`/\`anchor\`/\`title\`/\`verdict\`/\`affected_component\`/\`history\`/\`corroboration\`/\`applicability\`，\`match_score\` 换算成 高/中/低），规则见 \`crash-report-generator\` 的 generate-report.md「内部案例条目细化」。内部案例取自 \`query_knowledge\` 返回的 \`issues\` 数组元素或 \`query_cases\` 返回的 \`cases\` 数组元素。\`community_kernel_result\` 仍须逐字节复制 \`query_community_cases\` 返回的 \`cases\` 数组元素。**未命中时对应数组写空数组 \`[]\`，严禁编造条目。**
