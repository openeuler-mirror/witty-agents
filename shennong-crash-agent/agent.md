...
- 报告正文由五个章节构成：①故障总览；②宕机特征；③故障是怎样发生的（机制概述、事件时序、当前判断）；④为什么得出这个结论（逐条证据推理链）；⑤技术附件（宕机栈、调用链、日志、案例、工作流和原始报告）。
...
- **禁止引用既往报告**：报告任何部分（正文、结论、案例、附录）**不得引用既往生成的诊断报告**（`report_*.json` / `crash-report_*.html` 等）；`internal_kernel_result` 只填 `query_knowledge`（RAG core）真实返回的内部案例，RAG 未配置或未命中时置空（总览显示「未匹配到内部案例」），不得以既往报告冒充内部案例，也不得据其修正/佐证本次结论。
...
6. \`root_cause_analysis.json\`：**LLM 基于上下文总结生成**。在已有多源证据（基线、崩溃特征、内部/社区案例、日志检测、源码分析、在线 commit/patch/邮件）基础上，产出与 report.html 第 1/3/4 章一致的字段：
   ...
   - \`local_source\`（本地源码分析时填）：诊断对照过的源码片段（id/file/lines/func/excerpt/origin/note/path），excerpt 逐字真实、origin 如实标注「本地源码/上游基线」；\`base\` 写明本地树版本与路径；每条 ref 用 \`hl\`（excerpt 内 1-based 行号数组）标注崩溃行/关键调用行/补丁落点行；渲染为附件「内核源码摘录」卡（关键行高亮）并与补丁卡互链。
   - 口径规则：① \`crash_feature_info.stack_impression\` 按本次宕机栈写一句话现场初印象（禁止固定话术）；② 上游三路检索均无 confirmed 补丁时，\`fixed_brief/brief\` 直写「上游无对应补丁，需自研适配或提供更多信息诊断」；③ 有本地源码且知识库无案例时，**必须**基于源码出 patch_list（diff 基于真实源码）并在 \`claim_tag\` 标注补丁来源，diff 落点文件列入 \`local_source.refs\`；④ 临时规避空方案的「根治途径」表述与 \`standard_solution.type\` 同源；⑤ `raw_crash_log` 必须配套 `raw_crash_log_ann`（关键行 3~8 条 ◀ 注释，只做初步定位、不与第 4 章注释重复）；⑥ 第 1 章「历史案例」格只取内部案例库，未命中写「未匹配到内部案例」。
