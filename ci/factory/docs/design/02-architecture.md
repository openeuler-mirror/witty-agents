...
│  │ audit/ai/settings     │   │ ├ GitClient      (raw/API 只读)  │  │
...
要点：**三个上游都是"事实"，平台是"投影 + 控制面"**。平台可以触发 Jenkins，但不会绕开上游直接改状态；对 Git 仓库完全只读。
...
| Git 仓库 | `ci/agents.json`、`<agent>/ci/agent.json`、`<agent>/package.json`、分支与 commit 信息 | **不写** | 匿名 raw 读取（公开仓库）；私有仓库可选只读 Token | 10min 或手动触发 |
...
```text
节点（与 Jenkins 同机，架构不限）
├── Jenkins 容器 (127.0.0.1:8080)               ← 已有 / 由 ci/jenkins 部署
...
| 服务 | 宿主机端口 | 说明 |
| --- | --- | --- |
| factory-web | `127.0.0.1:18080` | 仅本机监听，经 SSH 隧道或前置代理访问；Jenkins 基线同样要求仅监听内网/127.0.0.1（见 [07](07-pipeline-integration.md) §12.3） |
...
| 平台 → Jenkins | 使用 Matrix 授权下的最小权限服务账号（权限矩阵见 [07](07-pipeline-integration.md) §12.2）；API Token 加密存储；Jenkins 地址仅内网可达 |
| 平台 → Git | **完全只读**：公开仓库匿名 raw 读取；私有仓库使用只读 scope Token；平台没有任何仓库写能力 |
...
FACTORY_GIT_TOKEN           Git 托管只读 Token（可选；仅私有仓库读取需要，公开仓库留空）
...
| # | 决策 | 结论 | 代价 |
| --- | --- | --- | --- |
| ADR-01 | 阶段与日志来源 | 主用 Jenkins `wfapi`（`/job/<j>/<n>/wfapi/describe`）；不可用时降级解析 `consoleText` 的 `[Pipeline]` 上下文 | 需要 pipeline-stage-view 插件（见 [07](07-pipeline-integration.md) §12.1 基线）；降级路径实现成本高 |
| ADR-02 | 阶段列表模板 | 平台内置 11 个阶段的名称与顺序模板，Jenkins 返回缺失的阶段显示为"未执行（条件阶段）" | 流水线新增阶段需同步模板（用版本号 + 校验兜底） |
| ADR-03 | 仓库写回 | 不做。平台对 Git 仓库完全只读；Agent 启停在仓库改 `ci/agents.json`，平台在下个同步周期反映 | 平台无"立即启停"能力（这是有意取舍，保证仓库唯一事实源） |
...
| ADR-10 | 服务账号权限 | Jenkins 采用 Matrix 授权，服务账号仅授予 Job 读/构建/取消等最小集合（见 [07](07-pipeline-integration.md) §12.2）；不授予 Administer / Configure / Credentials | 无法通过平台修改 Jenkins 自身配置（这是期望行为）；个别系统端点在最小权限下需降级容错 |
