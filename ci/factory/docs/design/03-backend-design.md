...
│   │   ├── git/
│   │   │   ├── client.ts            # GitCode/AtomGit REST（文件、分支、MR）
...
| agents | 从仓库同步 Agent 投影、启停 MR | `agents`、`agent_mrs` |
...
| 阶段列表 | `GET /job/<job>/<n>/wfapi/describe` | 11 阶段的状态与耗时；需 Workflow API |
| 阶段步骤 | `GET /job/<job>/<n>/execution/node/<nodeId>/wfapi/describe` | 按需展开 |
...
| `MrStatusSync` | 定时 | 60s | 跟踪平台创建的 MR 是否合入/关闭 |
...
| `agent.toggle_request` / `agent.mr_created` / `agent.mr_merged` | Agent 启停 | MR 链接、diff |
...
| Git API 失败 | Agent 启停不可用，其余功能不受影响 |
...
| `FACTORY_GIT_TOKEN` | 否 | — | 未配置则 Agent 启停 MR 功能禁用 |
